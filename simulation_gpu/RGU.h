//
// These are RG's utility functions to replace those from early versions
// of the NVIDIA SDK.  
//
...
if(err != CL_SUCCESS){
        fprintf(stderr,"eek clGetPlatformIDs, error id= %d\n", err);
	exit(1);
    	}
if(num_platforms == 0){
...
rval = fread((SourceString)+szPreamble,szSource,1,fptr); 
fclose(fptr);
